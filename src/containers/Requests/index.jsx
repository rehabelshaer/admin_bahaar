  ...
  return (
    <>
      ...
      <IndexMain
        ...
        // head table content
        addRowURL={getPermissions("requests", "x") ? config.routes.add : false}
        // head filters table
        filters={filters}
        // columns table content
        editRowURL={
          getPermissions("requests", "x") ? config.routes.edit : false
        }
        showRowURL={config.routes.show}
        deleteRow={{
          status: getPermissions("requests", "delete"),
          deleteRecord,
        }}
        action={
          getPermissions("requests", "delete") ||
          getPermissions("requests", "update") ||
          true
        }
        attachExport={false}
        attachExportParams={{}}
          ...
          "data",
          {
            ...
            content: (text, record, index) => {
              return (
                <div>
                  {record.status === 1 &&
                  getPermissions("assignRequest", "read") ? (
                    <Button
                      type="primary"
                      onClick={() => {
                        setUserData(record);
        ...
        options={
          getPermissions("changeStatus", "request") ||
          getPermissions("approve", "request") ||
          getPermissions("reject", "request")
            ? [
                {
                  title: "changeStatus".translate(),
                        ...
                        >
                          {getPermissions("approve", "request") && (
                            <Popconfirm
                              ...
                              okText={lang == "ar" ? "موافق" : "Ok"}
                              onConfirm={() =>
                                changeStatus(record, "approve?send=1")
                              }
                              cancelText={lang == "ar" ? "الغاء" : "Cancel"}
                            >
                              <Button
