  ...
  return (
    <>
      <IndexMain
        ...
        // head table content
        addRowURL={
          getPermissions("admins", "create") ? config.routes.add : false
        }
        // head filters table
        filters={filters}
        // columns table content
        editRowURL={
          getPermissions("admins", "update") ? config.routes.edit : false
        }
        showRowURL={config.routes.show}
        deleteRow={{ status: getPermissions("admins", "delete"), deleteRecord }}
        action={
          getPermissions("admins", "delete") ||
          getPermissions("admins", "update") ||
          true
        }
        generalCols={generateCols([
          ...
          "name_ar",
          {
            key: "categories",
            content: (text, record) => (
              <div>{record.categories.map((e) => e.name_en)}</div>
            ),
          },
          {
            key: "created_at",
